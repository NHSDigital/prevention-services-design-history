...
The programme needs a safe way to create clinics and book breast screening appointments for breast screening offices (BSOs) across the country. We had to decide where to begin quickly so that we could start making progress.
...
We chose to start with ‘clinics and appointment’ — where batches of people, time windows and locations become real clinics and bookable appointment slots. This aligns with the Manage team, whose focus is what happens during a mammography appointment.

We are starting at the point where planning becomes real, bookable clinics rather than at the earliest forecasting stage.
...
Today, there is no alternative to NBSS for creating mammography appointments. NBSS retirement is a hard dependency, so starting here reduces programme risk sooner.
...
### Practice variation narrows at this stage
...
This does not mean this is where we know the most. We expect to uncover edge cases, local workflows and operational constraints as we continue discovery.
