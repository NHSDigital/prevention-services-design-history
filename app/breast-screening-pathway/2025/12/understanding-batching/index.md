...
opengraphImage:
  src: /breast-screening-pathway/2025/12/understanding-batching/003-risp-batching-funnel.png
  alt: Funnel diagram illustrating how a Recall Interval Safety Period batch gets filtered down as the user sets parameters
---
...
Batching  is done in the Breast Screening Select (BS Select) system where BSOs can use  two main methods to create batches:
...
Participants with a non-batch referral screening episode in the last 12 months are not excluded from the RI/SP batch. 
...
- the date of their first offered appointment, or
- their screening episode date if a date of first offered appointment doesn’t exist

If the participant hasn’t been invited previously, their next test due date stays blank. 

For non-batch referral screening episodes, next test due dates will only be updated if the participant was screened. Their next test due date will be set to either three years after their actual screening date, or the episode date if an actual screening date doesn’t exist. 

#### How does NTDD batching work?

NTDD batching starts with the user selecting a next test due end date – this tells the batch to only include participants who are due before that date.
...
On the other hand, NTDD is more of an individual-led approach. An NTDD batch will only include participants who are due for screening before the chosen next test due end date. This helps avoid over-inviting participants and gives BSOs more flexibility to prioritise people who are due soon or already overdue, which can help with managing backlogs or unexpected changes. 
